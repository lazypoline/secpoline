...
extern "C" size_t meta_monitor(size_t arg1, size_t arg2, size_t arg3, size_t arg4, size_t arg5, size_t arg6, size_t syscall_no, size_t rip_after_syscall){
    ...
    if(syscall_no == __NR_read || syscall_no == __NR_recvfrom){
        if(arg1 == connect_fd){
            int virt_res = virtual_read(arg1, (void*)arg2, arg3, false);
            return virt_res;
        }
        return inline_syscall6(syscall_no, arg1, arg2, arg3, arg4, arg5, arg6);
        ...
        /*
        int res = inline_syscall6(syscall_no, arg1, arg2, arg3, arg4, arg5, arg6);
        if(virt_res == -1) return res;
        non_libc_print_str("starting proxyside read");
        non_libc_print_size_t("virt size", virt_res);
        non_libc_print_size_t("real size", res);
        nolibc_assert(virt_res==res);
        return res;
        */
    }

    if(syscall_no == __NR_write || syscall_no == __NR_sendto){
        if(arg1 == connect_fd){
            int virt_res = virtual_write(arg1, (void*)arg2, arg3, false);
            return virt_res;
        }
        return inline_syscall6(syscall_no, arg1, arg2, arg3, arg4, arg5, arg6);
        ...
        non_libc_print_str("starting proxyside write");
        non_libc_print_size_t("virt size", virt_res);
        non_libc_print_size_t("real size", res);
        nolibc_assert(virt_res==res);
        return res;
    ...
    if(syscall_no == __NR_poll){
        for(int i = 0;i< arg2; i++){
            struct pollfd fds = ((struct pollfd*)arg1)[i];
            int fd = fds.fd;
            if(fd == connect_fd){
                while(true){
                    int virt_ret = virtual_poll(fd, false);
                    if(virt_ret==0){
                        fds.revents = POLLIN;
                        ((struct pollfd*)arg1)[i] = fds;
                        return 1;
                    }

                    int real_ret = inline_syscall6(syscall_no, arg1, arg2, 0, arg4, arg5, arg6); //non-blocking version of poll
                    if(real_ret) return real_ret;
                }
            }
        }
