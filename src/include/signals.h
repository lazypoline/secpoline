...
#define SIZEOF_UCONTEXTV sizeof(ucontext_t)
...
public:
    SetRtSigProcMaskScope(const sigset_t& newmask) : 
        newmask{newmask}
    {
        auto result = nolibc_rt_sigprocmask(SIG_SETMASK, &newmask, &ourmask);
        if (result < 0) {
            errno = -result;
    ...
    ~SetRtSigProcMaskScope() {
		...
		auto result = nolibc_rt_sigprocmask(SIG_SETMASK, &ourmask, &dbg_newmask);
        assert(result == 0);
