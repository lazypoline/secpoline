...
#include <fcntl.h>
#include <pthread.h>
#include "../falco_shim/falco_shim.h"
...
int main(int argc, char* argv[]) {
    ...
    //at this point the monitor has been intialised

    //start falco
    pthread_t t1;
    pthread_create(&t1, NULL, shim_start, NULL);

    plugin_queue.wait_active();
...
extern "C" uint64_t secpoline_syscall_handler(long long* gp_regs) {
    ...
    assert(syscall_no < MAX_VALID_SYSCALL_NUM);
    send_syscall(syscall_no, gprs.arg1(), gprs.arg2(), gprs.arg3(), gprs.arg4(), gprs.arg5(), gprs.arg6());
    int ret;
